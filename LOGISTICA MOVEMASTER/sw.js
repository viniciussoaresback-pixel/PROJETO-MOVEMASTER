/* =====================================================================
   ...
   Estrat�gia: REDE PRIMEIRO, cache s� como reserva.
   Isso garante que o motorista sempre receba a vers�o mais nova do
   sistema quando tiver sinal, e ainda consiga abrir o app sem internet.

   IMPORTANTE: nada do Supabase � armazenado em cache � dados de carga,
...
// Instala��o: guarda o esqueleto do app
self.addEventListener('install', (evento) => {
  evento.waitUntil(
    caches.open(VERSAO)
      .then((cache) => cache.addAll(ARQUIVOS_BASE).catch(() => {
        // se algum arquivo falhar, n�o impede a instala��o
        return Promise.resolve();
      }))
  );
  // N�o chamamos skipWaiting aqui: a p�gina decide a hora de trocar,
  // para n�o recarregar em cima do motorista preenchendo algo.
});

// A p�gina avisa quando pode assumir
...
// Ativa��o: remove caches de vers�es antigas
...
self.addEventListener('fetch', (evento) => {
  ...
  if (ehAppShell) {
    // SEMPRE rede fresca (ignora o cache HTTP do navegador). Assim, o que voc�
    // publica aparece na hora. O cache serve s� de reserva quando est� offline.
    evento.respondWith(
  ...
  // Demais arquivos do app (�cones, manifest): rede primeiro, com cache de reserva
  evento.respondWith(
    fetch(req)
      .then((resposta) => {
...
/* ---------------------------------------------------------------------
   NOTIFICA��ES PUSH (preparado para uso futuro)
   No iOS, s� funcionam quando o app foi adicionado � Tela de In�cio.
   --------------------------------------------------------------------- */
self.addEventListener('push', (evento) => {
  let dados = { titulo: 'Movemaster', corpo: 'Voc� tem uma novidade.' };
